...
public class TurboExpCommand extends PlayerCommand {

    public TurboExpCommand(String permission) {
        super("turboexp", permission, "/ga turboexp <all/Tag> <Czas np. 5min>");
    }
    ...
    public boolean onCommand(Player player, String[] args) {
        ...
        if (args[1].equalsIgnoreCase("all")) {
            for (Guild guilds : PluginGuild.getPlugin().getGuildManager().getGuilds()) {
                guilds.setTurboexp(TimeUtil.getTimeWithString(args[2]));
            }
            Bukkit.getOnlinePlayers().stream().forEach(players -> TitleAPI.sendTitle(players, "&8[ &bTURBO&dEXP &8]", "&7Aktywowano kazdej gildii na &6" + TimeUtil.getDuration(TimeUtil.getTimeWithString(args[2])), 10, 70));
            return false;
        }
        ...
        guild.setTurboexp(TimeUtil.getTimeWithString(args[2]));
        for (GuildUser members : guild.getOnlineMembers()) {
            TitleAPI.sendTitle(members.getPlayer(), "&8[ &bTURBO&dEXP &8]", "&7Aktywowano Twojej gildii na &6" + guild.getDurationTurboExp() + "", 15, 70);
        }
        return true;
    }
}
