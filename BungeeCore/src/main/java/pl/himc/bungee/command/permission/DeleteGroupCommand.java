...
public final class DeleteGroupCommand extends Command {

    public DeleteGroupCommand(String permission) {
        super("deletegroup", permission);
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if(args.length != 2){
            ChatUtil.sendMessage(sender, messageConfiguration.usageCommand.replace("{USAGE}", "/bungee deletegroup <Nazwa grupy>"));
            return;
        }
        ...
        try {
            ...
            if (cfg.get("permissions." + args[1]) != null) {
                ...
                ChatUtil.sendMessage(sender, messageConfiguration.deleteGroupSuccess.replace("{GROUP}", args[1]));
            } else {
